...
void main()
{
	...
	const int train_class_cnt = 5;		//학습할 사람수
	const int train_face_cnt = 20;		//각 사람마다 학습할 얼굴 개수
	const Size n_face(70, 70);			//검출 얼굴의 정규영상
	...
	for(int i=0; i<train_class_cnt; i++) {
		...
		for(int j=0; j<train_face_cnt ; j++) {
			...
			//학습 얼굴영상 읽기
			char fname[100];
			sprintf(fname, "C:/Users/Administrator/Desktop/학생정보관리_프로그램/ourFirstProject/학생사진/face_%02d_%02d.png", i, j);
	...
	//수집 얼굴 학습
	//----------------------------------
	bool haveContribModul = initModule_contrib();		//예외 처리
	if(!haveContribModul) {
		cerr << "ERROR: The 'contrub' module is needed for ";
		cerr << "FaceRecognizer ut hasn't been loaded to OpenCV!" <<endl;
		exit(1);
	}
	...
	if(model.empty()) {
		...
		cerr << "Please ulpdate to OpenCV v2.4.1 or newer." <<endl;
		exit(-1);
	}
	...
	while(1) {
		...
		if(corrected_face.data)			//얼굴이 검출되면
		{
			...
			//얼굴 인식
			if(similarity < 1.0f) {
				identity = model->predict(corrected_face);		//얼굴 예측
				cout<<"얼굴번호 : "<<identity <<". 유사도 : "<<endl;
			...
			//인식 얼굴 우측 표시
				...
				cout<<"얼굴번호 :: Unknow. 유사도 : " <<similarity << endl;
			}
		}
		imshow("얼굴 검출", frame);

		if(waitKey(100) == 27) break;		//실행 중지 키 설정
	}
}